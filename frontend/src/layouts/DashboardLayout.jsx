...
const DashboardLayout = ({ type }) => {
  ...
  const getNavLinks = () => {
    if (type === 'candidate') {
      ...
      return [
        { to: '/company', icon: 'tachometer-alt', label: 'Dashboard' },
        ...
        <div className="fixed inset-y-0 left-0 flex flex-col w-64 bg-background-secondary shadow-custom-dark transition-transform duration-300 transform">
          <div className="flex items-center justify-between p-4 border-b border-dark-700">
            <div className="flex items-center">
              <FontAwesomeIcon icon="briefcase" className="text-primary-500 text-xl mr-2" />
              <span className="font-semibold text-white">TalentMatch</span>
            </div>
            <button 
              onClick={() => setSidebarOpen(false)} 
          ...
          <div className="p-4 border-t border-dark-700">
            <div className="flex items-center mb-4">
              <div className="w-10 h-10 rounded-full bg-primary-600 flex items-center justify-center text-white">
                <FontAwesomeIcon icon={type === 'candidate' ? 'user' : 'building'} />
              </div>
              <div className="ml-3">
