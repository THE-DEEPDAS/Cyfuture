...
export const calculateMatchScore = (job, resumeData) => {
  ...
  for (const jobSkill of jobSkills) {
    if (candidateSkills.some(skill => skill.includes(jobSkill) || jobSkill.includes(skill))) {
      skillMatchCount++;
    }
  }
  ...
  const skillMatchPercentage = jobSkills.length > 0 
    ? (skillMatchCount / jobSkills.length) * 100 
    : 0;
  ...
  if (resumeData.experience && resumeData.experience.length > 0) {
    ...
    const seniorTitles = ['senior', 'lead', 'principal', 'architect', 'manager', 'head'];
    ...
    if (jobExperience.includes('senior') && candidateLevel === 'senior') {
      experienceMatch = 100;
    } else if (jobExperience.includes('mid') && 
    ...
    } else if (candidateLevel === 'mid-level' && jobExperience.includes('junior')) {
  ...
  const overallScore = (skillMatchPercentage * skillWeight) + (experienceMatch * experienceWeight);
...
export const getLLMMatchAnalysis = async (job, resumeData) => {
  try {
    ...
    const prompt = `
      ...
      Candidate Experience: ${resumeData.experience.map(exp => 
        `${exp.title} at ${exp.company || 'Company'}`).join(', ')}
      Candidate Projects: ${resumeData.projects.map(proj => proj.name).join(', ')}
    ...
    const simulatedResponse = {
      ...
      explanation: `This candidate has ${resumeData.skills.filter(skill => 
        job.skills.some(jobSkill => 
          jobSkill.toLowerCase().includes(skill.toLowerCase()) || 
          skill.toLowerCase().includes(jobSkill.toLowerCase())
        )
      ).length} matching skills out of ${job.skills.length} required skills. Their experience as ${
    ...
    return {
      ...
      explanation: 'Match score calculated based on skills and experience alignment.'
    };
  }
};
      ...
      experience: [
        {
          ...
          endDate: null,
          description: 'Led a team of 5 developers to build a modern React-based application.'
        },
        {
          ...
          endDate: '2022-12-31',
          description: 'Developed and maintained multiple client applications using MERN stack.'
        }
      ],
      projects: [
        {
