    ...
    const fetchData = async () => {
      try {
        ...
        const applicationsData = [
          {
            ...
            resume: {
              ...
              fileUrl: '#',
              parsedData: {
                skills: ['JavaScript', 'React', 'HTML', 'CSS', 'Redux', 'TypeScript'],
                experience: [
                  {
                    title: 'Frontend Developer',
                    company: 'TechCorp',
                    ...
                    endDate: null,
                    description: 'Developing modern web applications using React'
                  },
                  {
            ...
            matchScore: 92,
            llmRationale: 'The candidate has strong React skills and relevant experience in frontend development. Their projects demonstrate capability with modern web technologies.',
            status: 'shortlisted',
            messages: [
              {
                _id: 'm1',
                sender: 'company',
                content: 'Hi John, we were impressed with your application. Are you available for an interview next week?',
                createdAt: '2025-03-16T10:00:00Z'
              },
              {
                _id: 'm2',
                sender: 'candidate',
                content: 'Hello, yes I am available. I can do Monday or Wednesday afternoon.',
          ...
          },
          {
            ...
            resume: {
              ...
              fileUrl: '#',
              parsedData: {
                skills: ['JavaScript', 'React', 'CSS', 'SASS', 'Webpack', 'Jest'],
                experience: [
                  {
                    title: 'UI Developer',
                    company: 'DesignCo',
            ...
            coverLetter: 'I believe my design background and development skills make me a great fit...',
            matchScore: 88,
            llmRationale: 'The candidate has solid React experience with a good design background, which is valuable for a frontend role. Their portfolio shows clean code and attention to UI details.',
            status: 'reviewing',
            messages: [],
          ...
          },
          {
            ...
            matchScore: 95,
            llmRationale: 'The candidate has exceptional experience with Node.js and database technologies that perfectly align with our requirements. Their background in building scalable systems is particularly valuable.',
            status: 'shortlisted',
            messages: [],
        ...
        if (jobId) {
          ...
          setApplications(applicationsData.filter(app => app.job._id === jobId));
        } else {
      ...
      <div className="lg:w-1/4">
        <div className="card sticky top-24">
          ...
          <div className="space-y-6">
            <div>
              <label htmlFor="jobFilter" className="block text-sm font-medium text-gray-300 mb-2">
                Job Posting
              </label>
              <select
                id="jobFilter"
      ...
      <div className="lg:w-2/5">
        <div className="card mb-6">
          <h2 className="text-xl font-semibold text-white mb-4">
            Applications {selectedJob !== 'all' ? `for ${jobs.find(j => j._id === selectedJob)?.title}` : ''}
          </h2>
          ...
          <div className="space-y-4">
            {getFilteredApplications().map(application => (
              <div
                ...
                className={`bg-background-secondary rounded-lg p-4 cursor-pointer transition-all hover:shadow-lg ${
                  selectedApplication?._id === application._id ? 'ring-2 ring-primary-500' : ''
                }`}
                onClick={() => viewApplication(application)}
              >
                <div className="flex justify-between">
                  <h3 className="font-medium text-white">{application.candidate.name}</h3>
            ...
            {getFilteredApplications().length === 0 && (
              <div className="bg-background-secondary rounded-lg p-6 text-center">
                <FontAwesomeIcon icon="file-alt" className="text-3xl text-gray-600 mb-2" />
                <p className="text-gray-400">No applications found for the selected filters.</p>
              </div>
            )}
          </div>
        </div>
        ...
        {selectedApplication ? (
          <div className="card sticky top-24 overflow-y-auto max-h-[calc(100vh-120px)]">
            <div className="mb-6">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-gray-400">{selectedApplication.job.title}</p>
                </div>
                <span className={getMatchScoreBadge(selectedApplication.matchScore)}>
                  {selectedApplication.matchScore}% Match
                </span>
              </div>
              
              <div className="mt-4 flex flex-wrap gap-2">
                {selectedApplication.resume.parsedData.skills.map((skill, index) => (
                  <span 
                    key={index}
            ...
            <div className="p-4 bg-background-secondary rounded-lg mb-6">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex items-start">
                  <FontAwesomeIcon icon="envelope" className="text-primary-500 mt-1 mr-3" />
                  <span className="text-gray-300">{selectedApplication.candidate.email}</span>
                </div>
                <div className="flex items-start">
                  <FontAwesomeIcon icon="phone" className="text-primary-500 mt-1 mr-3" />
                  <span className="text-gray-300">{selectedApplication.candidate.phone}</span>
                </div>
              </div>
            </div>
            ...
            <div className="mb-6">
              ...
              <div className="space-y-4">
                {selectedApplication.resume.parsedData.experience.map((exp, index) => (
                  <div key={index} className="border-l-2 border-primary-700 pl-4 ml-2">
                    <div className="font-medium text-white">{exp.title}</div>
                    <div className="text-gray-300">{exp.company}</div>
                    <div className="text-sm text-gray-400">
                      {new Date(exp.startDate).getFullYear()} - {exp.endDate ? new Date(exp.endDate).getFullYear() : 'Present'}
                    </div>
                    <div className="mt-1 text-sm text-gray-300">{exp.description}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="mb-6">
              ...
              <div className="space-y-4">
                {selectedApplication.resume.parsedData.education.map((edu, index) => (
                  <div key={index} className="border-l-2 border-primary-700 pl-4 ml-2">
            ...
            <div className="mb-6">
              ...
              <div className="flex flex-wrap gap-2">
                <button 
                  onClick={() => updateStatus(selectedApplication._id, 'pending')}
                ...
                <button 
                  onClick={() => updateStatus(selectedApplication._id, 'reviewing')}
                  className={`px-3 py-1 rounded-md text-sm ${
          ...
          <div className="card text-center py-12">
            <FontAwesomeIcon icon="user" className="text-5xl text-gray-600 mb-4" />
            <h3 className="text-lg font-medium text-white">No Candidate Selected</h3>
