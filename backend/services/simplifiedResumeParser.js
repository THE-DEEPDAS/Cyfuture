    ...
    for (const header of sectionHeaders) {
      if (
        ...
        if (
          ...
          /PROJECT|PORTFOLIO|DEVELOPMENT|APPLICATIONS/i.test(matchedHeader)
        ) {
          sectionType = "PROJECTS";
        } else if (
          /EDUCATION|ACADEMIC|DEGREE|UNIVERSITY|COLLEGE|SCHOOL/i.test(
            matchedHeader
          )
        ) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (
        ...
        (/SKILL|TECH|LANGUAGE|TOOL|EXPERT|COMPETENC|PROFICIEN|QUALIF/i.test(
          ...
          /PROJECT|PORTFOLIO|APPLICATION|SYSTEM|DEVELOPMENT|HOBBY|INTEREST/i.test(
            line
          ) ||
...
function extractSkills(skillsSection) {
  ...
  for (const line of skillsSection) {
    ...
    if (
      ...
      if (
        cleanLine.includes(":") &&
        cleanLine.indexOf(":") < cleanLine.length / 2
      ) {
        ...
        const skillsInLine = skillsList
          ...
          .filter(
            (skill) => skill.length > 1 && skill.split(/\s+/).length <= 3
    ...
    // Process line with a category followed by skills (e.g., "Programming Languages: Python, Java")
      ...
      if (
        /\b(training|internship|hobbies|interests|education|experience|project)\b/i.test(
          category
        )
      ) {
        continue;
  ...
  return [...new Set(allSkills)]
    .filter((skill) => {
      ...
      const containsHeaderWords =
        /\b(TRAINING|INTERNSHIP|HOBBIES|INTERESTS|EXPERIENCE|PROJECT|EDUCATION)\b/i.test(
          skill
        );
...
function extractExperience(experienceSection) {
  ...
  if (experiences.length === 0) {
    ...
    const positionRegex =
      /\b(developer|engineer|intern|assistant|manager|leader|contributor|trainee)\b/i;
...
function applyContentBasedClassification(lines, sectionContent) {
  ...
  if (unclassifiedLines.length > 0) {
    ...
    for (const cluster of unclassifiedClusters) {
      ...
      for (
        let i = Math.max(0, clusterStartIdx - contextRange);
        i < clusterStartIdx;
        i++
      ) {
      ...
      for (
        let i = clusterEndIdx + 1;
        i <= Math.min(allLines.length - 1, clusterEndIdx + contextRange);
        i++
      ) {
        const section = lineToSectionMap.get(allLines[i]);
        if (section === "SKILLS") nearbySkills++;
      ...
      for (const item of cluster) {
        ...
        if (
          /\b(senior|junior|lead|developer|engineer|manager|director)\b/i.test(
            item.line
          )
        ) {
      ...
      if (
        hasDatePattern &&
        (hasCompanyOrTitlePattern || nearbyExperience > 0)
      ) {
        ...
        hasDatePattern &&
        (nearbyProjects > 0 ||
          /\b(github|project|application|website)\b/i.test(
            cluster.map((i) => i.line).join(" ")
          ))
      ) {
        ...
        if (highestContext > 0) {
          ...
          else if (nearbyExperience === highestContext)
            targetSection = "EXPERIENCE";
          else if (nearbyProjects === highestContext)
            targetSection = "PROJECTS";
          else if (nearbyEducation === highestContext)
