...
export const generateInitialInterview = async (job) => {
  const prompt = `
    ...
    Experience Level: ${job?.experience?.years || "Not specified"} years in ${job?.experience?.field || "relevant field"}
    
    Your response should include:
    1. A brief introduction as an AI interviewer for this company (1-2 sentences)
    2. A welcome message explaining this is an automated interview process (1 sentence)
    3. 3-5 relevant interview questions that assess the candidate's fit for this role
    
    Format your questions clearly, with one question per paragraph. Ask specific questions that will help evaluate
    the candidate's skills, experience, and fit for this particular role. Each question should be relevant to
  ...
  try {
    ...
    interviewCache.set(cacheKey, response);
    ...
    return "Hello! I'm the AI interviewer for this position. Could you tell me about your relevant experience and skills for this role?";
  }
};
 ...
 */
export const evaluateCandidateResponse = async (application, candidateResponse, messageHistory) => {
  ...
  const cacheKey = `response-eval-${application._id}-${candidateResponse.substring(0, 50)}`;
  if (interviewCache.has(cacheKey)) {
    return interviewCache.get(cacheKey);
  }
  ...
  try {
    ...
    try {
      ...
      if (!result) {
        ...
        const evaluationMatch = aiResponse.match(/evaluation["']?\s*:\s*["']([^"']+)["']/i);
        const questionMatch = aiResponse.match(/followUpQuestion["']?\s*:\s*["']([^"']+)["']/i);
        
        result = {
          evaluation: evaluationMatch ? evaluationMatch[1] : "Response evaluated.",
          score: scoreMatch ? parseInt(scoreMatch[1]) : 70,
          followUpQuestion: questionMatch ? questionMatch[1] : "Can you tell me more about your experience?"
        };
      }
    } catch (parseError) {
      ...
      result = {
        ...
        score: 70,
        followUpQuestion: "Can you elaborate on your skills and experience related to this position?"
      };
    }
    ...
    return {
      ...
      score: 70,
      followUpQuestion: "Could you tell me more about how your skills align with this position?"
    };
  }
};
 ...
 */
export const updateMatchScoreFromInterview = async (application, evaluations) => {
  if (!evaluations || evaluations.length === 0) {
    return application.matchScore;
  }
  
  // Calculate average interview score
  const avgInterviewScore = evaluations.reduce((sum, evaluation) => sum + evaluation.score, 0) / evaluations.length;
  ...
  const updatedScore = Math.round(currentMatchScore * 0.7 + avgInterviewScore * 0.3);
 ...
 */
export const generateRejectionExplanation = async (job, candidate, matchScore) => {
  const prompt = `
    ...
    - Title: ${job.title}
    - Required Skills: ${job.requiredSkills?.join(", ") || "Not specified"}
    - Experience Needed: ${job.experience?.years || "Not specified"} years in ${job.experience?.field || "relevant field"}
    - Education Required: ${job.education?.level || "Not specified"} in ${job.education?.field || "relevant field"}
    
    Candidate Profile:
    - Skills: ${candidate.skills?.join(", ") || "Not provided"}
    - Experience: ${candidate.experience ? JSON.stringify(candidate.experience) : "Not provided"}
    - Education: ${candidate.education ? JSON.stringify(candidate.education) : "Not provided"}
  ...
  try {
    ...
    return `We appreciate your interest in this position. While your profile has several strengths, we're looking for candidates with more specific experience in ${job.requiredSkills?.join(", ") || "the required areas"}. We encourage you to continue developing your skills in these areas and wish you success in your job search.`;
  }
};
