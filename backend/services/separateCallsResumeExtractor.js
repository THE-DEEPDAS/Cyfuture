/**
 * LLM-based Resume Extractor with Separate Calls
 *
 * This module uses an LLM to extract structured information from resume text.
 * It makes separate calls to extract skills, experience, and projects independently.
 */
...
${resumeText}`; // Call the LLM with the prompt - no delay for first call
...
Resume text to parse:
${resumeText}`; // Call the LLM with the prompt - force delay to avoid throttling
...
async function extractResumeDataWithLLM(resumeText) {
  try {
    console.log("Starting LLM-based resume extraction with separate calls"); // Extract skills first
    console.log("Step 1: Extracting skills");
    const skills = await extractSkillsWithLLM(resumeText);
    console.log(`Extracted ${skills.length} skills`);

    // Extract experience next with forced delay to avoid API throttling
    console.log("Step 2: Extracting experience");
    const experience = await extractExperienceWithLLM(resumeText);
    console.log(`Extracted ${experience.length} experience items`);

    // Extract projects last with forced delay to avoid API throttling
    console.log("Step 3: Extracting projects");
    const projects = await extractProjectsWithLLM(resumeText);
    console.log(`Extracted ${projects.length} projects`);
...
async function parseResumeWithLLM(pdfBuffer) {
  try {
    ...
    // Extract data using LLM with separate calls
