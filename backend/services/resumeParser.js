...
export const parseResume = async (pdfBuffer) => {
  ...
  if (
    !llmApiAvailable &&
    currentTime - lastLlmApiCheck < LLM_API_RETRY_INTERVAL
  ) {
    console.log(
      "Skipping LLM parsing due to previous API availability issues, using traditional parsing..."
    );
  ...
  try {
    ...
    if (llmResults.skills && llmResults.skills.length > 0) {
      console.log(
        `LLM parsing successful: Found ${llmResults.skills.length} skills, ${llmResults.experience.length} experiences, ${llmResults.projects.length} projects`
      );
    ...
    console.log(
      "LLM parsing did not return skills, falling back to traditional parsing..."
    );
    return await parseResumeText(pdfBuffer);
  } catch (error) {
