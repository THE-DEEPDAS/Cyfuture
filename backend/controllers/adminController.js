...
const getAdminDashboardStats = asyncHandler(async (req, res) => {
  ...
  const averageMatchScore = matchScoreCount > 0 ? (totalMatchScore / matchScoreCount).toFixed(2) : 0;
...
const getAdminJobs = asyncHandler(async (req, res) => {
  ...
  const jobsWithCounts = jobs.map(job => {
    ...
    const shortlistedCount = job.applications.filter(app => app.status === 'Shortlisted').length;
...
export { getAdminDashboardStats, getAdminJobs, updateCompanyProfile };